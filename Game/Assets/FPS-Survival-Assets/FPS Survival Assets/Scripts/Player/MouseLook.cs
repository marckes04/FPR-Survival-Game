...
public class MouseLook : MonoBehaviour
{
    ...
    void LookAround()
    {
        current_Mouse_Look = new Vector2(Input.GetAxis(Axis.MOUSE_VERTICAL), Input.GetAxis(Axis.MOUSE_HORIZONTAL));

        look_Angles.x += current_Mouse_Look.x * sensivity * (invert ? 1f : -1f);
        look_Angles.y += current_Mouse_Look.y * sensivity;

        //look_Angles.y = look_Angles.y + current_Mouse_Look.y * sensivity;

        look_Angles.x = Mathf.Clamp(look_Angles.x, default_Look_Limits.x, default_Look_Limits.y);

        current_Roll_Angle = Mathf.Lerp(current_Roll_Angle, Input.GetAxisRaw(Axis.MOUSE_HORIZONTAL)
